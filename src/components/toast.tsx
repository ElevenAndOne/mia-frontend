    ...
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 max-w-sm">
      {toasts.map(toast => {
        ...
        return (
          <div
            key={toast.id}
            className={`border rounded-lg p-4 shadow-lg animate-in slide-in-from-right-full duration-200 ${styles.container}`}
            role="alert"
          >
            <div className="flex items-start gap-3">
              <IconComponent className={`w-5 h-5 ${styles.icon} shrink-0 mt-0.5`} />
              <p className="flex-1 paragraph-sm text-primary">{toast.message}</p>
              <button
                type="button"
