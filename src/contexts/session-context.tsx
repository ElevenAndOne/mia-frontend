...
// eslint-disable-next-line react-refresh/only-export-components -- useSession hook must be co-located with SessionContext for proper context usage
...
export const SessionProvider: React.FC<SessionProviderProps> = ({ children }) => {
          ...
          if (sessionResponse.ok) {
            ...
            if (data.valid) {
              ...
              if (workspacesResponse.ok) {
                ...
                availableWorkspaces = (workspacesData.tenants || []).map((t: Workspace) => ({
  ...
  const refreshWorkspaces = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        const workspaces: Workspace[] = (data.tenants || []).map((t: Workspace) => ({
          tenant_id: t.tenant_id,
          name: t.name,
          slug: t.slug,
          role: t.role,
  ...
  const selectAccount = async (accountId: string, industry?: string): Promise<boolean> => {
    ...
    try {
      ...
      const requestBody: { account_id: string; session_id: string | null; industry?: string } = {
