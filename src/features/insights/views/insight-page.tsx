      ...
      <div key={index}>
        <div className="bg-primary border border-secondary rounded-lg p-5 space-y-3">
          ...
          <div className="flex items-start gap-3">
            <div
              className={`shrink-0 w-7 h-7 ${config.colors.badge} text-primary-onbrand rounded-full flex items-center justify-center label-sm`}
            >
              {index + 1}
            </div>
            <h3 className="flex-1 label-md text-primary leading-snug">{insight.title}</h3>
          </div>
          ...
          {(insight.action || (isCurrentCard && currentSection === 'action')) && (
            <div
              className={`pl-10 ${config.colors.actionBg} border-l-4 ${config.colors.actionBorder} p-3 rounded`}
            >
              <p
                className="paragraph-sm text-primary leading-relaxed"
