...
import DateRangeSelector from '../dashboard/date-range-selector'
import { useStreamingInsightsParsed, ParsedInsight } from './use-streaming-insights-parsed'

// Helper component to render text with markdown links and campaign deep links
const MarkdownText = ({ text, className = '', metaAdsId }: { text: string; className?: string; googleAdsId?: string; metaAdsId?: string }) => {
...
interface OptimizeInsightsStreamingProps {
  onBack?: () => void
  initialDateRange?: string
  platforms?: string[]
}

const OptimizeInsightsStreaming = ({ onBack, initialDateRange = '30_days', platforms }: OptimizeInsightsStreamingProps) => {
    ...
    return (
      <div key={index} className="bg-white border border-gray-200 rounded-lg p-5 space-y-3">
        ...
        {(insight.action || (isCurrentCard && currentSection === 'action')) && (
          <div className="pl-10 bg-blue-50 border-l-4 border-blue-500 p-3 rounded">
      ...
      <div className="flex-1 bg-white p-6 pb-32 overflow-y-auto rounded-t-2xl -mt-4">
        ...
        {insights.length > 0 && !error && (
          <div className="space-y-6">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Key Optimization Opportunities</h2>
...
export default OptimizeInsightsStreaming
