    ...
    <div className="flex h-full min-h-0">
      <div className="flex flex-col h-full min-h-0 flex-1 min-w-0">
        ...
        <div className="flex justify-end px-1 pt-1">
          <BuildHistoryMenu
            builds={c.pastBuilds}
            onLoadList={c.openHistory}
            onSelect={c.loadPastBuild}
            onNew={c.startFresh}
          />
        </div>
        ...
        {c.pdfUploading && (
          <div className="flex flex-col items-center justify-center gap-3 py-20">
            ...
            <p className="paragraph-sm text-secondary">
              Parsing your campaign brief — this may take a minute…
            </p>
          </div>
        )}
        ...
        {!c.pdfUploading && !started && (
          <div className="text-center pt-12 pb-6 px-6">
            ...
            <p className="paragraph-sm text-tertiary mb-6">
              Chat with Mia to build your first campaign, or upload a brief.
            </p>
            <button
              onClick={() => c.send('Build a new campaign')}
              className="px-3 py-1.5 border border-secondary rounded-full paragraph-sm text-secondary hover:bg-secondary transition-colors"
            >
            ...
            <button
              ...
              className="mt-3 inline-flex items-center gap-2 px-4 py-2 border border-secondary rounded-full paragraph-sm text-tertiary hover:bg-secondary hover:text-primary transition-colors"
            >
        ...
        {!c.pdfUploading && started && (
          ...
          >
            {c.messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] px-4 py-3 rounded-2xl paragraph-sm leading-relaxed ${m.role === 'user' ? 'bg-brand-solid text-primary-onbrand' : 'bg-secondary text-primary border border-secondary'}`}
                >
                  {m.role === 'user' ? (
        ...
        {!c.pdfUploading && (
          <ChatComposer
            ...
            onRemoveDoc={c.removePendingDoc}
            onPasteLarge={c.addPastedText}
          />
        )}
      </div>
      ...
      {showCanvas && (
        <div className="hidden md:block w-[45%] max-w-[45rem] h-full shrink-0">
          <BuilderCanvas
