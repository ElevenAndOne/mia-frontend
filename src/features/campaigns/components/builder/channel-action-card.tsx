...
const OSA: Array<{
  field: 'objective' | 'strategy' | 'action_notes'
  label: string
  ask: string
  placeholder: string
}> = [
  ...
  { field: 'strategy', label: 'Strategy', ask: 'channel strategy', placeholder: 'Add strategy…' },
  {
    field: 'action_notes',
    label: 'Action',
    ask: 'channel tactics / action plan',
    placeholder: 'Add tactical actions, posting schedule, content mix…',
  },
]
const inputCls =
...
export const ChannelActionCard = ({
  phaseId,
  phaseName,
  action,
  currency,
  ...
  const { patchAction, addAsset, patchAsset, deleteAsset } = useChannelEditor(
    phaseId,
    action.action_id
  )
      ...
      <div className="flex items-center">
        {hasPicker && (
          ...
          >
            ...
            <span
              className={`label-xs font-semibold ${linked.length ? 'text-utility-brand-600' : 'text-utility-brand-500'}`}
            >
        ...
        <div
          className={`flex items-center gap-2 ${hasPicker ? 'pl-0' : 'pl-3'} pr-3 py-2.5 flex-1 cursor-pointer min-w-0`}
          onClick={() => setExpanded(!expanded)}
        >
          {!hasPicker && (
            <span className="label-xs text-utility-brand-700 bg-utility-brand-100 px-2 py-0.5 rounded-full shrink-0">
              {label}
            </span>
          )}
          {budget.amount != null && (
            <span className="paragraph-xs text-tertiary shrink-0 cw-mono">
              {formatBudget(budget.amount, currency)}
              {budget.period === 'total' ? ' total' : '/mo'}
            </span>
          )}
          ...
          {linked.length > 0 && (
            <div className="flex flex-wrap gap-1 max-w-[40%]">
              {linked.slice(0, 2).map((c) => (
                <span
                  key={c.id}
                  className="paragraph-xs text-quaternary bg-tertiary px-1.5 py-0.5 rounded truncate max-w-[7.5rem]"
                  title={c.name}
                >
                  {c.name}
                </span>
        ...
        {confirmingRemove ? (
          ...
          <button
            ...
            className="p-1 mr-1 text-quaternary hover:text-utility-error-500 shrink-0"
        ...
        <div className="pr-3 py-2.5 cursor-pointer shrink-0" onClick={() => setExpanded(!expanded)}>
          <svg
            className={`w-3.5 h-3.5 text-quaternary transition-transform ${expanded ? 'rotate-180' : ''}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="p-3 space-y-3 border-t border-tertiary">
          {OSA.map(({ field, label: lbl, ask, placeholder }) => (
            <div key={field}>
              <div className="flex items-center justify-between mb-1">
                ...
                <AskMiaButton
                  context={{ fieldLabel: ask, phaseName, channel: label }}
                  currentValue={action[field] ?? ''}
                  onInsert={(t) => patchAction({ [field]: t })}
                />
              </div>
              <EditableTextarea
                value={action[field] ?? ''}
                onSave={(v) => patchAction({ [field]: v || null })}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {budget.derived ? (
                <div className="px-2 py-1.5 border border-tertiary rounded-lg bg-primary">
                  <span className="text-xs text-secondary font-medium cw-mono">
                    {formatBudget(budget.amount, currency)}{' '}
                    {budget.period === 'total' ? 'total' : '/mo'}
                  </span>
                  <p className="paragraph-xs text-quaternary mt-0.5">
                    Sum of {budget.assetCount} asset{budget.assetCount === 1 ? '' : 's'} · edit per
                    asset below
                  </p>
                </div>
              ) : (
                <div className="flex items-center gap-1">
                  <input
                    type="number"
                    key={`${action.action_id}-b-${action.budget ?? ''}`}
                  ...
                  <select
                    ...
                    onChange={(e) => patchAction({ budget_period: e.target.value })}
                    className="shrink-0 px-1.5 py-1.5 border border-tertiary rounded-lg text-xs bg-secondary-subtle text-secondary outline-none"
                  >
            ...
            <div>
              ...
              <div className="space-y-1">
                <input
                  type="date"
                  key={`${action.action_id}-sd-${action.start_date ?? ''}`}
                  defaultValue={action.start_date ?? ''}
                  onChange={(e) => {
                    if (e.target.value !== (action.start_date ?? ''))
                      patchAction({ start_date: e.target.value || null })
                  }}
                  className={inputCls}
                />
                <input
                  type="date"
                  key={`${action.action_id}-ed-${action.end_date ?? ''}`}
                  defaultValue={action.end_date ?? ''}
                  onChange={(e) => {
                    if (e.target.value !== (action.end_date ?? ''))
                      patchAction({ end_date: e.target.value || null })
                  }}
                  className={inputCls}
                />
              </div>
          ...
          <div>
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform ${assetsOpen ? 'rotate-180' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <div className="mt-2 space-y-2">
                {action.assets.map((asset) => (
                  <AssetCard
                    ...
                    channel={label}
                    phaseName={phaseName}
                    onPatch={(f) => patchAsset(asset.asset_id, f)}
                    onDelete={() => deleteAsset(asset.asset_id)}
                  />
                ))}
                <button
                  onClick={addAsset}
                  className="w-full py-2 border border-dashed border-secondary rounded-lg label-xs text-quaternary hover:text-secondary hover:border-tertiary transition-colors"
                >
                  + Add asset
                </button>
              </div>
              ...
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M9 12l2 2 4-4m-9.5-4.5h11a1.5 1.5 0 011.5 1.5v11a1.5 1.5 0 01-1.5 1.5h-11A1.5 1.5 0 015 19V7a1.5 1.5 0 011.5-1.5z"
                  />
                </svg>
                Review &amp; push
              </button>
