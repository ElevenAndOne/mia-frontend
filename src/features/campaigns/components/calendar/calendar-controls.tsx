  ...
  return (
    <div className="flex items-center justify-between gap-5 flex-wrap">
        ...
        {channels.map((c) => {
          ...
          return (
            ...
            >
              ...
              />
              <span
                className="paragraph-xs font-medium"
                style={{ color: on ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)' }}
              >
                {c.label}
              </span>
              <span
                className="cw-mono text-[0.6563rem] font-semibold rounded-full px-1.5"
                style={{
                  color: on ? c.color : 'var(--color-text-quaternary)',
                  background: on ? softColor(c.color, 16) : 'var(--color-background-tertiary)',
                }}
              >
                {monthCounts[c.channel] ?? 0}
              </span>
            </button>
          )
        })}
      ...
      <div className="flex items-center gap-2.5">
        <button
          onClick={onPrev}
          onDragOver={dragFlip(onPrev)}
          disabled={prevDisabled}
          title="Previous month — hover here while dragging a post to move it across months"
          className="w-8 h-8 rounded-lg border border-secondary flex items-center justify-center text-tertiary hover:bg-secondary disabled:opacity-35"
        >
          ‹
        </button>
        <div className="min-w-[9.375rem] text-center">
        ...
        <button
          onClick={onNext}
          onDragOver={dragFlip(onNext)}
          disabled={nextDisabled}
          title="Next month — hover here while dragging a post to move it across months"
          className="w-8 h-8 rounded-lg border border-secondary flex items-center justify-center text-tertiary hover:bg-secondary disabled:opacity-35"
        >
          ›
        </button>
