    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex">
      <div className="w-1/2 p-6 border-r border-slate-700 flex flex-col">
        ...
        <div className="bg-slate-900/80 border border-slate-700 rounded-xl p-4 flex-1 overflow-auto">
          <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
            <History className="w-4 h-4" /> History ({editHistory.length})
          </h3>
      ...
      <div className="w-1/2 p-6 flex flex-col">
        <div className="bg-slate-900/80 border border-slate-700 rounded-xl p-4 flex-1 flex flex-col">
          <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
            <MessageCircle className="w-4 h-4" /> Edit Instructions
          </h3>
          <div className="flex-1 bg-slate-800/50 rounded-lg p-4 overflow-y-auto mb-4">
            <div className="text-sm text-slate-300 mb-4">
              Tell me what to change about the image:
            </div>
            <div className="space-y-2">
              {[
                'remove the hat and add sunglasses',
                ...
                <button
                  ...
                  className="block w-full text-left text-xs text-slate-400 hover:text-purple-400 hover:bg-slate-700/50 rounded p-2 transition-colors"
                >
                  "{ex}"
                </button>
              ))}
...
export default function ImagineTab({
  ...
  useEffect(() => {
    creativeStudioApi
      .listCampaigns(tenantId, sessionId)
    ...
    const poll = setInterval(async () => {
      try {
        ...
        if (job.status === 'completed') {
          ...
          setEditHistory((prev) => [
            ...prev,
            { image: newUrl, prompt: 'Edit applied', timestamp: new Date() },
          ])
        ...
        pending.map(async (v) => {
          try {
            ...
            if (job.status === 'completed') {
              updates.push({
                ratio: v.ratio,
                url: job.output_urls?.[0] ?? job.output_url ?? null,
                status: 'completed',
              })
            } else if (job.status === 'failed') {
      ...
      ALL_RATIOS.filter((r) => r !== aspectRatio).map(async (ratio) => {
        try {
          ...
          if (res.job_id) {
            setFormatVariants(
              (prev) =>
                prev?.map((v) => (v.ratio === ratio ? { ...v, jobId: res.job_id } : v)) ?? null
            )
          } else {
            setFormatVariants(
              (prev) =>
                prev?.map((v) => (v.ratio === ratio ? { ...v, status: 'failed' } : v)) ?? null
            )
          }
        } catch {
          setFormatVariants(
            (prev) => prev?.map((v) => (v.ratio === ratio ? { ...v, status: 'failed' } : v)) ?? null
          )
        }
      })
    )
  ...
  const isCampaignScopedResult =
    isCampaignScoped && intelligence?.campaign_id === selectedCampaignId
  const scopeLabel = isCampaignScoped
  ...
  return (
    <>
      <IterativeEditingInterface
        ...
        currentImage={currentEditImage}
        editHistory={editHistory}
        onEdit={handleIterativeEdit}
        isEditing={isEditing}
      ...
      <div className="grid grid-cols-12 gap-6 items-stretch">
            ...
            <div className="space-y-2">
              {campaigns.length > 0 ? (
                    ...
                    {selectedCampaignId ? (
                      <>
                        ...
                        <span className="text-purple-400">
                          Campaign objectives will guide prompt optimization
                        </span>
                      </>
                    ) : (
                      <>
                        ...
                        <span className="text-green-400">
                          Brand guide active — select a campaign to narrow context
                        </span>
                      </>
                    )}
                  </p>
                ...
                <p className="text-xs text-green-400 flex items-center gap-1">
                  <Sparkles className="w-3 h-3" /> Brand guide active — prompt optimization will use
                  your workspace brand context
                </p>
              )}
            </div>
          </div>
        ...
        <div className="col-span-6 flex flex-col gap-4">
            ...
            <div className="shrink-0 flex items-center gap-3 px-4 py-2.5 bg-purple-500/10 border border-purple-500/30 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-xs text-slate-400 ml-2 truncate">
                  sourced from: "{variantSeed.prompt.slice(0, 60)}
                  {variantSeed.prompt.length > 60 ? '…' : ''}"
                </span>
              </div>
              <button
                onClick={onClearVariantSeed}
                className="p-1 hover:bg-slate-700 rounded text-slate-500 hover:text-white transition-colors shrink-0"
              >
          ...
          <div className="flex-1 bg-slate-900/80 backdrop-blur-sm border border-slate-700 rounded-xl p-4 min-h-[17.5rem] flex items-center justify-center">
            {/* Format grid view */}
            {formatVariants ? (
              <div className="w-full">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    onClick={() => setFormatVariants(null)}
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {formatVariants.map((v) => (
                    <div
                      key={v.ratio}
                      className="relative bg-slate-800 rounded-lg overflow-hidden aspect-video flex items-center justify-center group"
                    >
                      {v.status === 'completed' && v.url ? (
                        <>
                          ...
                          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <a
                              href={v.url}
                              download={`creative_${v.ratio.replace(':', 'x')}.png`}
                      ...
                      <div className="absolute top-2 left-2 px-1.5 py-0.5 bg-slate-900/80 text-slate-300 text-[0.625rem] font-semibold rounded">
                        {v.ratio}
                      </div>
                    </div>
                  ))}
                ...
                {generatedImages.map((img, idx) => (
                  <div key={idx} className="relative group">
                    ...
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-2">
                      ...
                      {supportsEditing && (
                        <button
                          onClick={() => handleStartEditing(img.url)}
                          className="p-2 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-lg"
                          title="Edit iteratively"
                        >
                          <Edit3 className="w-4 h-4" />
                        </button>
                      )}
              ...
              <div className="text-center">
                {isGenerating ? (
                  ...
                  <>
                    <div className="w-10 h-10 text-slate-600 mx-auto mb-3 flex items-center justify-center">
          ...
          <div className="flex-1 flex flex-col bg-slate-900/80 backdrop-blur-sm border border-slate-700 rounded-xl p-4">
            ...
            {!intelligence ? (
              ...
              <div className="flex items-center gap-2 text-blue-400 text-xs">
                ...
                {isCampaignScoped ? ` ${scopeLabel}` : ' your top Meta ads'}...
              </div>
            ) : intelligence.status === 'completed' ? (
              <div className="space-y-2">
                ...
                >
                  {isCampaignScoped && !isCampaignScopedResult
                    ? `Analyze ${scopeLabel}`
                    : 'Re-analyze'}
                </button>
              </div>
              ...
              <div className="space-y-2">
                <p className="text-xs text-red-400">
                  {intelligence.error_message || 'Analysis failed'}
                </p>
                <button
                  onClick={handleRefreshIntelligence}
                  disabled={intelligenceLoading}
                ...
                >
                  {intelligenceLoading ? (
                    ...
                    <>
                      ...
                      {isCampaignScoped ? `Analyze ${scopeLabel}` : 'Analyze Ad Performance'}
                    </>
                  )}
                </button>
              </div>
