...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1 px-2 py-1 rounded-full border text-xs ${styles[status] ?? styles.failed}`}
    >
      {icons[status]}
...
function LightboxModal({
  ...
  return (
    ...
    >
      ...
      <div
        className="max-w-5xl max-h-[90vh] w-full flex flex-col items-center gap-4"
          ...
          <button
            onClick={() =>
              downloadAsset(
                ...
                sessionId,
                asset.asset_id,
                asset.filename || `${asset.media_type}_${asset.asset_id}`,
                asset.cdn_url
              )
            }
      ...
      <div className="relative aspect-video bg-slate-800">
        ...
        {asset.cdn_url && (
          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
            ...
            <button
              onClick={() =>
                downloadAsset(tenantId, sessionId, asset.asset_id, dlFilename, asset.cdn_url)
              }
        ...
        <div className="absolute top-3 right-3">
          <div
            className={`text-xs px-2 py-1 rounded-full ${isVideo ? 'bg-purple-500/80 text-white' : 'bg-blue-500/80 text-white'}`}
          >
            {isVideo ? 'Video' : 'Image'}
          </div>
        </div>
              ...
              <div className="absolute right-0 bottom-full mb-2 bg-slate-800 border border-slate-700 rounded-lg shadow-xl py-2 w-36 z-10">
                ...
                {!isVideo && asset.cdn_url && (
                  <button
                    onClick={() => {
                      setShowMenu(false)
                      onCreateVariant?.(asset.prompt ?? '', asset.cdn_url)
                    }}
    ...
    <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden hover:border-purple-500/50 transition-all group">
      <div className="relative aspect-square bg-slate-800">
        ...
        <div className="absolute top-2 right-2">
          <span
            className={`text-xs px-2 py-1 rounded-full font-medium ${isFigma ? 'bg-purple-500/80 text-white' : 'bg-slate-700/90 text-slate-300'}`}
          >
...
function FigmaImportModal({
  ...
  tenantId: string
  sessionId: string
  onImported: (assets: CreativeAsset[]) => void
  onClose: () => void
}) {
    ...
    try {
      ...
      setFilesError(
        msg.includes('not connected')
          ? 'Figma is not connected. Go to Settings → Integrations to connect.'
          : msg
      )
    } finally {
  ...
  const selectFile = async (file: FigmaFile) => {
    ...
    try {
      ...
      if ((res as any).unsupported) {
        setImportError(
          'Could not read this file. Try reconnecting your Figma account in Settings → Integrations.'
        )
      }
    } catch {
      setFrames([])
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {step === 'frames' && selectedFile && (
                <p className="text-slate-400 text-xs mt-0.5 truncate max-w-[17.5rem]">
                  {selectedFile.name}
                </p>
              )}
            </div>
              ...
              {filesLoading ? (
                <div className="flex flex-col items-center justify-center h-32 gap-2">
                  ...
                  <p className="text-slate-500 text-xs">
                    Loading all projects — may take a few seconds...
                  </p>
                </div>
              ) : filesError ? (
                ...
                <div className="px-5 py-8 text-center">
                  ...
                  <p className="text-slate-400 text-sm">
                    {query ? 'No files match your search' : 'No Figma files found'}
                  </p>
                </div>
              ) : (
        ...
        {step === 'frames' && (
          <>
            ...
            {importError && <div className="px-5 py-2 text-red-400 text-xs">{importError}</div>}
            ...
            <div className="px-5 py-4 border-t border-slate-700 flex items-center justify-between">
              ...
              >
                {importing ? (
                  ...
                  <>
                    Import {selected.size > 0 ? selected.size : ''} frame
                    {selected.size !== 1 ? 's' : ''}
                  </>
                )}
              </button>
...
export default function LibraryTab({
  ...
  sessionId,
  onCreateVariant,
}: Props & { onCreateVariant?: (prompt: string, imageUrl: string) => void }) {
  ...
  return (
    <div>
            ...
            <div className="flex gap-1 bg-slate-800 rounded-lg p-1">
              {(['all', 'image', 'video'] as MediaFilter[]).map((f) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-colors ${mediaFilter === f ? 'bg-purple-500 text-white' : 'text-slate-400 hover:text-white'}`}
                >
                  {f}
                </button>
              ))}
      ...
      <div className="flex items-center justify-between mb-4">
        {activeSubTab === 'library' ? (
          <p className="text-slate-400 text-sm">
            Showing {filteredGenerated.length} of {generatedAssets.length} assets
          </p>
        ) : (
          <p className="text-slate-400 text-sm">
            {referenceAssets.length} reference {referenceAssets.length === 1 ? 'asset' : 'assets'}
          </p>
        )}
        {activeSubTab === 'library' && (
          <div className="flex gap-2">
            {(['grid', 'list'] as ViewMode[]).map((m) => (
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${viewMode === m ? 'bg-purple-500 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
              >
                {m === 'grid' ? (
        ...
        (generatedLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-400 mb-2">
              {generatedAssets.length === 0 ? 'No assets yet' : 'No assets match your filters'}
            </p>
            <p className="text-slate-500 text-sm">
              {generatedAssets.length === 0
                ? 'Generate your first image or video in the Create or Imagine tabs'
      ...
      {activeSubTab === 'assets' &&
        (referenceLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-500 text-sm">
              Import frames from Figma to use as references in your generations
            </p>
            <button
              onClick={() => setShowFigmaModal(true)}
