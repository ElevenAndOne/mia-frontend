...
const BronzeCardV2: React.FC<BronzeCardV2Props> = ({
  ...
  const platformStyles: Record<string, { icon: string; iconBg: string }> = {
    ...
    meta_ads: { icon: '/icons/meta-color.svg', iconBg: 'bg-white' },
    ga4: { icon: '/icons/google_analytics.svg', iconBg: 'bg-white' },
  }
...
const OnboardingChatV2: React.FC<OnboardingChatV2Props> = ({
  ...
  // Suppress unused variable warning - onSkip may be used in future
  void _onSkip
  ...
  const handleInsightChoice = async (_type: 'grow' | 'optimise' | 'protect') => {
