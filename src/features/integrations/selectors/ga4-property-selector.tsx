...
const GA4PropertySelector = ({
  ...
  const fetchGA4Properties = async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      logger.log(
        '[GA4-PROPERTY-SELECTOR] Properties not cached yet, polling for background fetch...'
      )
  ...
  const handleLinkProperties = async () => {
    ...
    try {
      ...
      if (selectedPropertyIds.length === 0) {
        logger.log(
          '[GA4-PROPERTY-SELECTOR] Unlinking all GA4 properties from account',
          selectedAccount.name
        )
      } else {
        logger.log(
          '[GA4-PROPERTY-SELECTOR] Linking',
    ...
    >
      ...
      <div className="space-y-4">
        ...
        {isLoading && (
          <div className="py-8 text-center">
            ...
            <p className="mt-1 paragraph-xs text-quaternary">
              This can take up to 20 seconds on first connect
            </p>
          </div>
        )}
        ...
        {error && !isLoading && (
          <div className="bg-error-primary border border-error-subtle rounded-lg p-4">
            <div className="flex items-start">
              <svg
                className="w-5 h-5 text-error mt-0.5 mr-3 shrink-0"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
          ...
          <>
                  ...
                  return (
                    <div
                      ...
                      className={`relative p-4 rounded-lg border-2 transition-all ${
                        isSelected ? 'border-brand bg-brand-primary' : 'border-secondary bg-primary'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {isSelected && (
                            <svg
                              className="w-3 h-3 text-primary-onbrand"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path
            ...
            <div className="bg-brand-primary border border-utility-brand-200 rounded-lg p-3">
              <p className="paragraph-xs text-brand-primary">
                💡 Select one or more GA4 properties to link to your{' '}
                {currentAccountName || 'Google Ads'} account for website analytics.
              </p>
            </div>
          </>
        )}
        ...
        {!isLoading && !error && properties.length === 0 && (
          <div className="py-8 text-center">
            <svg
              className="w-16 h-16 mx-auto text-placeholder-subtle mb-4"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
