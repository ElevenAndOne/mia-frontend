...
const MetaAccountSelector = ({ isOpen, onClose, onSuccess, currentGoogleAccountName, currentAccountData }: MetaAccountSelectorProps) => {
  ...
  const fetchMetaAccounts = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load Meta accounts. Please try again.'
  ...
  const handleLinkAccount = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to update Meta account. Please try again.'
      setError(errorMessage)
    } finally {
      setIsLinking(false)
    }
