  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || 'Failed to connect Brevo account')
      }
    } catch (err) {
      console.error('Brevo connection error:', err)
      const errorMessage = err instanceof Error ? err.message : 'Failed to connect to Brevo. Please check your API key.'
