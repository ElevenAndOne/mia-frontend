...
const GA4PropertySelector = ({ isOpen, onClose, onSuccess, currentAccountName, ga4Properties, linkedProperties }: GA4PropertySelectorProps) => {
  ...
  const fetchGA4Properties = async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load GA4 properties. Please try again.'
  ...
  const handleLinkProperties = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to link GA4 property. Please try again.'
      setError(errorMessage)
    } finally {
      setIsLinking(false)
    }
