...
interface IntegrationStatusResult {
  ...
  currentAccountData: Record<string, unknown> | null
...
async function fetchIntegrationStatus(
  ...
  let currentAccountData: Record<string, unknown> | null = null
  ...
  if (accountsResponse.ok) {
    ...
    if (accountsData.accounts && accountsData.accounts.length > 0) {
      const account = selectedAccountId
        ? accountsData.accounts.find((acc: Record<string, unknown>) => acc.id === selectedAccountId)
