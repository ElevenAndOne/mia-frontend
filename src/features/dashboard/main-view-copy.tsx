...
import { usePlatformPreferences } from './use-platform-preferences'
import BrevoConnectionModal from '../integrations/selectors/brevo-connection-modal'
...
const MainViewCopy = ({ onLogout: _onLogout, onIntegrationsClick, onWorkspaceSettingsClick, onGrowQuickClick, onOptimizeQuickClick, onProtectQuickClick }: MainViewProps) => {
  ...
  const connectedPlatforms = useMemo(() => {
    ...
    return platformConfig.filter(p => {
      const value = (selectedAccount as Record<string, unknown>)[p.accountKey]
      return value && value !== ''
    }).map(p => p.id)
