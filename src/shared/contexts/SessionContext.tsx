...
export const SessionProvider: React.FC<SessionProviderProps> = ({ children }) => {
          ...
          if (sessionResponse.ok) {
            ...
            if (data.valid) {
              ...
              if (workspacesResponse.ok) {
                ...
                availableWorkspaces = (workspacesData.tenants || []).map((t: { tenant_id: string; name: string; slug: string; role: 'owner' | 'admin' | 'analyst' | 'viewer'; onboarding_completed: boolean; connected_platforms?: string[]; member_count?: number }) => ({
  ...
  const refreshWorkspaces = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        const workspaces: Workspace[] = (data.tenants || []).map((t: { tenant_id: string; name: string; slug: string; role: 'owner' | 'admin' | 'analyst' | 'viewer'; onboarding_completed: boolean; connected_platforms?: string[]; member_count?: number }) => ({
          tenant_id: t.tenant_id,
          name: t.name,
          slug: t.slug,
          role: t.role,
