...
import { useGoogleCampaigns } from '../../features/onboarding/use-google-campaigns';

export function GoogleCampaignsScreen() {
  const { selectedAccountId, selectedCampaignIds, selectCampaigns, nextStep, back } = useOnboarding();
  const { campaigns, status } = useGoogleCampaigns(selectedAccountId);
